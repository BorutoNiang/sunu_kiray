...
def _fix_rdv(rows: list) -> list:
    """Convertit timedelta et date en string pour la sérialisation JSON."""
    for r in rows:
        if isinstance(r.get("heure_rdv"), datetime.timedelta):
            s = int(r["heure_rdv"].total_seconds())
            r["heure_rdv"] = f"{s//3600:02d}:{(s%3600)//60:02d}:00"
...
def index(page: int = 1, per_page: int = 20, statut: Optional[str] = None,
    ...
    try:
        ...
        with db.cursor() as cur:
            cur.execute(
                f"SELECT COUNT(*) AS total FROM rendez_vous r "
                f"JOIN medecins md ON r.medecin_id=md.id "
                f"JOIN services sv ON r.service_id=sv.id "
                f"JOIN structures_sante st ON sv.structure_id=st.id WHERE {w}", params)
...
# ── DATES DISPONIBLES DU MOIS ─────────────────────────────

@router.get("/dates-disponibles")
def dates_disponibles(structure_id: int = Query(...), service_id: int = Query(...),
                      annee: int = Query(...), mois: int = Query(...),
                      _=Depends(get_current_user)):
    """Retourne toutes les dates du mois ayant au moins un créneau libre."""
    db = get_db()
    try:
        ...
        with db.cursor() as cur:
            cur.execute("""
                SELECT DISTINCT date_travail AS date_dispo
                FROM disponibilites
                WHERE structure_id=%s AND service_id=%s
                  AND date_travail >= %s AND date_travail < %s
                  AND date_travail >= CURDATE()
                  AND est_actif=1 AND nb_slots_pris < nb_slots_max
                ORDER BY date_travail
            """, (structure_id, service_id, debut, fin))
            rows = cur.fetchall()
        dates = [str(r['date_dispo']) if isinstance(r['date_dispo'], datetime.date) else r['date_dispo'] for r in rows]
